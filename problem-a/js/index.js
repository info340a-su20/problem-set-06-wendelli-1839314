...
function renderTrack(song) {
  ...
  newElement.src = song.artworkUrl100;
  newElement.alt = song.trackName;
  newElement.title = song.trackName;
  records.appendChild(newElement);
}
...
function renderSearchResults(object) {
  ...
  let musicTrack = object.results;
  if (musicTrack.length == 0) {
    renderError(new Error("No results found"));
  }
  for (let i = 0; i < musicTrack.length; i++) {
    renderTrack(musicTrack[i]);
  }
}
